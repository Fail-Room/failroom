...
Failroom has no application runtime, terminal gateway, learner sandbox runtime, authentication system, or production deployment. Phase 1 includes a pure profile qualification gate, a verified diagnostic Docker primitive, a transactional SQLite state store, a trusted in-process control-plane composition, a bounded HMAC capability codec and an atomic attachment-lease contract. The diagnostic path may inspect and clean a real container only from a trusted operator context with explicit inputs; the codec and lease tests do not prove HTTP authentication, learner runtime isolation or PTY security, and Linux end-to-end evidence is UNVERIFIED in the current Windows environment. Other controls in this document remain mandatory requirements for future implementation. Runtime claims require test evidence from the phase that introduces them.
...
There is no learner application runtime to inspect or test. Qualification decisions, the verified diagnostic Docker adapter, the trusted control-plane composition, explicit SQLite migration through v3, the bounded capability codec and SQLite authority/cleanup-intent plus attachment-lease persistence are implemented. The adapter denies non-pinned images, declared image volumes, unsafe seccomp policy files, profile drift, unexpected mounts and ownership mismatch; the control-plane path records lifecycle state only after ordered transitions and verified Docker evidence. The codec has no HTTP issuer or key-distribution service, and its opt-in integration test requires a trusted Linux controller and explicit operator inputs, so it is unverified by default here. Evidence collection for all required learner checks, final-image qualification, authentication, PTY handling, background scheduling, independent TTL enforcement and physical cleanup of learner resources remain unimplemented. SQLite storage does not enforce TTL while services are down.

The in-process terminal gateway coordinator now links one authenticated backend capability consumption to one generation-bound control-plane attachment lease. It does not provide HTTP authentication, WebSocket transport, PTY handling, or physical isolation evidence.
