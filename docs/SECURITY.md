...
Failroom has no application runtime, terminal gateway, learner sandbox runtime, authentication system, or production deployment. Phase 1 includes a pure profile qualification gate, a verified diagnostic Docker primitive, a transactional SQLite state store, and a trusted in-process control-plane composition. The diagnostic path may inspect and clean a real container only from a trusted operator context with explicit inputs; its tests do not prove learner runtime isolation or signed-token security, and its Linux end-to-end evidence is UNVERIFIED in the current Windows environment. Other controls in this document remain mandatory requirements for future implementation. Runtime claims require test evidence from the phase that introduces them.
...
There is no learner application runtime to inspect or test. Qualification decisions, the verified diagnostic Docker adapter, the trusted control-plane composition, explicit SQLite migration and SQLite authority/cleanup-intent persistence are implemented. The adapter denies non-pinned images, declared image volumes, unsafe seccomp policy files, profile drift, unexpected mounts and ownership mismatch; the control-plane path records lifecycle state only after ordered transitions and verified Docker evidence. Its opt-in integration test requires a trusted Linux controller and explicit operator inputs, so it is unverified by default here. Evidence collection for all required learner checks, final-image qualification, authentication, PTY handling, background scheduling, independent TTL enforcement and physical cleanup of learner resources remain unimplemented. SQLite storage does not enforce TTL while services are down.
