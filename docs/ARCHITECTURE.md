...
This document defines the architecture for Failroom. Phase 0 established documentation and repository rules. Phase 1 implements an internal qualification evaluator and verified diagnostic Docker lifecycle in services/sandbox-engine/, SQLite repositories and an injected cleanup pass in packages/state-store/, a trusted in-process control-plane composition in services/control-plane/, a bounded signed capability codec and backend authority facade in services/api/, and an in-process terminal gateway authority coordinator. The control plane owns explicit configuration, state-before-runtime ordering, operator-only migration and evidence-gated cleanup for this diagnostic slice; the state store also owns the consumed attachment-lease contract. No web/API application, terminal transport, learner sandbox runtime, authentication system, or deployment is implemented.
...
The first implementation target is one secure browser-to-sandbox terminal path. Next.js, xterm.js, and FastAPI remain planned architecture choices. The current Phase 1 implementation proves the repository authority and trusted state-to-diagnostic-Docker composition, including explicit migration, immutable runtime bindings, ordered create/start transitions, durable cleanup wiring, a bounded HMAC capability codec, backend capability authority, terminal gateway consume-to-lease coordination, and an atomic attachment-lease contract. HTTP authentication endpoints, WebSocket transport, browser/API integration, PTY attachment, independent TTL enforcement and learner sandbox access remain planned and unimplemented. Phase 2 generalizes this baseline into reset, multiple concurrent sandboxes, the complete reusable lifecycle state machine and transition/race matrices, and broader reconciliation. The component boundaries below are logical contracts; Phase 1 may colocate processes, but it must preserve the same authorization and trust boundaries.
The API package also provides an in-process `BackendCapabilityAuthority` that
composes owner-filtered attempt inspection, capability issuance and atomic `jti` introspection; HTTP transport and external authentication remain planned.

## Implemented Trusted Control-Plane Boundary

services/control-plane is the only package that composes the state store with sandbox-engine Docker primitives. LifecycleOrchestrator records CREATING and STARTING before Docker side effects, records READY only after post-start evidence, and preserves exact attempt, sandbox, generation and runtime-operation bindings. The operator-only migrate command refuses implicit v1 changes and requires a new absolute backup path. The API contract package issues and verifies bounded HMAC capabilities but does not expose an HTTP listener or transport authentication.

DockerCleanupRuntime passes the persisted original runtime-operation binding to the sandbox lifecycle, while DockerCleanupWorker records DESTROYED only after verified absence evidence. The Linux Docker integration is opt-in and requires a trusted Linux host; a skipped run on Windows is UNVERIFIED evidence, not a passing isolation result.

## Implemented Persistence Boundary

The [state store](../packages/state-store/README.md) uses five SQLite tables: backend-owned `room_attempts` and `terminal_capability_uses`, control-plane-owned `sandbox_resources` and `terminal_attachment_leases`, and an operation-owner `lifecycle_operations` log. Backend preallocation, ownership checks, exact tuple matching, compare-and-set resource transitions, immutable TTL, epoch revocation, request-fingerprint idempotency and atomic consumed-jti hashing are implemented at the repository boundary. Resource and attempt updates remain separate even though their transactions use one private local database.

These APIs require already authenticated contexts and verified capability claims from trusted callers; they do not implement authentication or signature verification. A trusted Clock is sampled after obtaining the writer lock. Resource readiness and destruction accept trusted evidence references, not sandbox assertions. Receipts are historical database results and cannot authorize allocation or attachment. DockerCleanupWorker performs one bounded pass; the trusted control-plane adapter supplies the runtime verifier but does not schedule itself or provide transport authentication. Attachment lease consumption is implemented at the state boundary, while lease transport, PTY attachment and independent TTL enforcement remain unimplemented.

Cleanup intent and retry records survive reopening. Control-plane reconciliation enumerates reserved cleanup tuples, including interrupted creates without a resource row. Backend finalization has its own recovery query for interruptions after resource destruction was recorded. The diagnostic adapter performs exact Docker binding, profile and absence checks only for trusted operator calls; its physical Linux evidence remains opt-in and unverified in the current Windows environment. Learner access still requires the missing authentication, attachment, PTY and full isolation integration. Store database and journal files outside synchronized source checkouts and sandbox-accessible paths.
...
The following shape is the target layout. The sandbox qualification, state-store and API contract modules now exist; HTTP application and runtime directories remain planned:
